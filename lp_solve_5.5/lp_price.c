...
int CMP_CALLMODEL compareImprovementQS(const QSORTrec *current, const QSORTrec *candidate)
{
  return( compareImprovementVar((pricerec *) current->self, (pricerec *) candidate->self) );
}
int CMP_CALLMODEL compareSubstitutionQS(const QSORTrec *current, const QSORTrec *candidate)
{
...
STATIC MYBOOL multi_resize(multirec *multi, int blocksize, int blockdiv, MYBOOL doVlist, MYBOOL doIset)
{
  ...
  if((blocksize > 1) && (blockdiv > 0)) {
    ...
    multi->sortedList = (QSORTrec *) realloc(multi->sortedList, (multi->size+1)*sizeof(*(multi->sortedList)));
...
STATIC int multi_getvar(multirec *multi, int item)
{
  ...
  if((item < 1) || (item >= multi->size))
    return(-1);
#endif
  return( ((pricerec *) &(multi->sortedList[item]))->varno );
}
