...
STATIC REAL presolve_round(lprec *lp, REAL value, MYBOOL isGE)
{
  ...
  REAL testin  = value + my_chsign(isGE, PRESOLVE_EPSVALUE/SCALEDINTFIXRANGE),
...
STATIC MYBOOL presolve_probefix01(presolverec *psdata, int colnr, REAL *fixvalue)
{
  ...
  for(ix = presolve_nextrow(psdata, colnr, &item);
    ...
    *fixvalue = COL_MAT_VALUE(ix);
    ...
    canfix = (MYBOOL) ((loLim + fabs(*fixvalue) > lp->orig_rhs[i]+lp->epsvalue));
    ...
    if(!canfix && !my_infinite(lp, get_rh_range(lp, i))) {
      ...
      loLim = my_chsign(!chsign, loLim);
      *fixvalue = -(*fixvalue);
      canfix = (MYBOOL) ((loLim + fabs(*fixvalue) > get_rh_range(lp, i)-lp->orig_rhs[i]+lp->epsvalue));
    }
  }
...
STATIC int presolve_probetighten01(presolverec *psdata, int colnr)
{
  ...
  for(ix = presolve_nextrow(psdata, colnr, &item); ix >= 0;
    ...
    if(upLim - fabs(value) < lp->orig_rhs[i]-lp->epsvalue) {
...
int CMP_CALLMODEL compRedundant(const QSORTrec *current, const QSORTrec *candidate)
{
...
int CMP_CALLMODEL compSparsity(const QSORTrec *current, const QSORTrec *candidate)
{
  int start1 = (int) (current->prev),
      start2 = (int) (candidate->prev),
      result = CMP_COMPARE(start1, start2);
...
int CMP_CALLMODEL compAggregate(const QSORTrec *current, const QSORTrec *candidate)
{
...
{
  lprec    *lp = psdata->lp;
  MYBOOL   candelete, GUBactive = is_bb_mode(lp, NODE_GUBMODE);
...
STATIC int presolve_freeandslacks(presolverec *psdata, int *nCoeffChanged, int *nConRemove, int *nVarFixed, int *nSum)
{
  ...
  for(j = firstActiveLink(psdata->cols->varmap); j != 0; ) {
    ...
    if((presolve_collength(psdata, j) != 1) || is_int(lp, j) || is_semicont(lp, j) || !presolve_candeletevar(psdata, j)) {
      j = nextActiveLink(psdata->cols->varmap, j);
      continue;
    }
    ix = 0;
...
STATIC int presolve_rows(presolverec *psdata, int *nCoeffChanged, int *nConRemove, int *nVarFixed, int *nBoundTighten, int *nSum)
{
  ...
  REAL     Value1, Value2, losum, upsum, lorhs, uprhs;
  MATrec   *mat = lp->matA;
...
/*            psdata->forceupdate = (MYBOOL) (iSum > 0);*/
            nBoundTighten += iBoundTighten;
            iBoundTighten  = 0;
            nSum          += iSum;
            iSum           = 0;
