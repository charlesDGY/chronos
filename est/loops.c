...
int isInner(int lpId, int pLpId) {
	...
	return 0;
#endif
	loop_t* lca = loop_comm_ances[lpId][pLpId];
	if (lca->id == pLpId) return 1;
	else return 0;
}
...
int cmpLpOrder(int lpId1, int lpId2) {
#if 1
	/*
	 * NOTE: This implementation should not be in case the tcfg is unrolled
	 */
	...
	if (lpId1 == lpId2)
		return 0; //same lpId -> sameOrder
	if (lpId1 == 0)
		return 1; //lpId==0 --> main program (outer most loop)
	if (lpId2 == 0)
		return -1; //lpId2 is outer loop of lpId1
	else if (lpId1 > lpId2)
		return 1;
	else
		return -1;
...
static int exit_loop(tcfg_edge_t *e) {
	loop_t *lp;
...
static void reg_loop_exit(tcfg_edge_t *e) {
	loop_t *lp;
	tcfg_elink_t *elink;
...
static void deal_exit_edge(tcfg_edge_t *e) {
	loop_t *lp;
	tcfg_node_t *dst;
	...
	if (loop_map[dst->id] != NULL) {
		...
		if (loop_map[dst->id]->parent != NULL
			...
			return;
		for (lp = loop_map[e->src->id]->parent; lp != loops[0];
				lp = lp->parent) {
			head_bid = bbi_bid(lp->head);
			tail_bid = bbi_bid(lp->tail);
...
static void deal_other_edge(tcfg_edge_t *e) {
	if (loop_map[e->dst->id] == NULL) {
...
static void search_common_ancestor(loop_t *x, loop_t *y) {
	loop_t *lp;
	int i, flag = (int) x;
...
void loop_relations() {
	...
	for (i = 0; i < num_tcfg_loops; i++)
		loop_comm_ances[i] = (loop_t **) calloc(num_tcfg_loops,
				sizeof(loop_t *));
	...
	for (i = 0; i < num_tcfg_nodes; i++) {
		...
		if (y == tcfg[0])
			...
			printf("%d.%d: \t%d[%d.%d - %d.%d]  / parent:%d\n", bbi_pid(x),
					bbi_bid(x), loop_map[i]->id, bbi_pid(y), bbi_bid(y),
					bbi_pid(z), bbi_bid(z), loop_map[i]->parent->id);
	}
}
