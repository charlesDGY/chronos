...
int getIbLB(inf_node_t *ib) {
	if (ib->loop_id == -1)
		...
		return inf_loops[ib->loop_id].bound;
}
...
void enum_regular_address(dat_inst_t* d_inst, expr_p expr, int flag, int curEq,
		worklist_p curTSnode, int curAddr) {
	int dbg = 0;
	int i;
	saddr_p smem;
	ts_p ts;
	...
	if (curEq < expr->varNum) {
		...
		if (lastNode) {
			...
			ADD_ADDR: smem = createSAddr(GET_MEM(curAddr), getAddrD(d_inst),
					flag, *enumTSset);
...
int analyze_regular_access(dat_inst_t *d_inst, inf_node_t *ib) {
	...
	for (i = 0; i < exp->varNum; i++) {
		...
		for (j = i + 1; j < exp->varNum; j++) {
			if (exp->value[j].val <= exp->value[min].val)
				min = j;
#if 0
	...
	if (dbg) {
		fprintf(dbgAddr, "\nGenerated range: [%x, %x], %d elems", minAddr,
				maxAddr, GET_MEM(maxAddr-minAddr));
...
int analyze_unpred_access(dat_inst_t *d_inst, inf_node_t* ib) {
	...
	if (foundRange) { //unpredictable access, but find global var
		...
		if (gVar->size > CACHE_SIZE
		)
			goto UNKNOWN_RANGE;

		if (dbg) {
			fprintf(dbgAddr, "\n Global var: %s [%x,%x], array sa: %x, size %d",
					gVar->name, gVar->addr, gVar->addr + gVar->size, initAddr,
					gVar->size);
			fflush(dbgAddr);
		}
		for (addr = gVar->addr; addr < gVar->addr + gVar->size; addr += 4) {
			if (curblk && GET_MEM(addr) == curblk->blkAddr)
				continue;
			memblk = createSAddr(GET_MEM(addr), hexValue(insn->addr), 0, orgTS);
			addAfterNode(memblk, &blkNode, &(d_inst->addr_set));
			curblk = memblk;
		}
	} else { //unpredictable access, unknown address range
		UNKNOWN_RANGE: if (dbg) {
			fprintf(dbgAddr, "\nUnknown addr range");
			fflush(dbgAddr);
		}
...
int analyze_scalar_access(dat_inst_t *d_inst, inf_node_t* ib) {
	...
	if (d_inst->addrExpr.varNum != 0) {
		printf("\nERR: not scalar access");
		printDataRef(stdout, d_inst);
	}
...
void genAccessAddress(dat_inst_t *d_inst, inf_node_t* ib) {
	...
	insn = d_inst->insn;
	extern int enable_write_allocate;
...
void readExtraCons(char *bin_fname) {
	...
	while (fgets(str, 256, fcons) != 0) {
		...
		if (dbg) {
			fprintf(dbgAddr, "Cons Type %s", token);
			fflush(dbgAddr);
		}
		if (strcmp(token, "eql") == 0) {
			...
			if (isInner(idL1, idL2) == 0) {
				printf("\nWrong RLB cons: L%d is not inner loop of L%d", l1->id,
						l2->id);
				exit(1);
			}
			l1->rId = l2->id;
...
void initAll(char *bin_fname) {
	...
	for (i = num_tcfg_loops - 1; i > 0; i--) {
		...
		if (loop->rId == -1) {
			if (loop->parent)
				loop->exec = loop->parent->exec * loop->bound;
		} else {
			...
			case INV_LB:
				...
				break;
			default:
				printf("\nUnknown relative bound cons type: %d", loop->rType);
				fflush(stdout);
				exit(1);
				break;
			}
	...
	if (dbg) {
		for (i = 1; i < num_tcfg_loops; i++) {
			loop = loops[i];
			if (loop->rId == -1) {
				fprintf(dbgAddr, "\n Loop L%d lb:%d entry:%d exec:%d", loop->id,
						loop->bound, loop->parent->exec, loop->exec);
			} else {
				preLoop = loops[loop->rId];
				switch (loop->rType) {
				case EQL_LB:
...
void readSizeCons(char *bin_fname) {
	...
	while (!feof(gArray)) {
		...
		if (dbg)
			fprintf(dbgAddr, "\nSize constraint: %s %d", sCons->name,
					sCons->size);
	}
}
void classified_address_analysis(char *bin_fname) {
	...
	for (i = 0; i < prog.num_procs; i++) {
		...
		for (j = 0; j < ip->num_bb; j++) {
			ib = &(ip->inf_cfg[j]);
			if (dbg) {
				fprintf(dbgAddr, "\nBB (%d,%d) L%d  num:%d  sa:%s", i, j,
						ib->loop_id, ib->num_insn, ib->insnlist[0].addr);
				fflush(dbgAddr);
			}
			bb = ib->bb;
	...
	if (1) {
		fprintf(dbgAddr, "Created memblk: %d\n", totalBlk);
		fflush(dbgAddr);
		printf("Created memblk: %d\n", totalBlk);
		fflush(stdout);
	}
	freeMem();
}
