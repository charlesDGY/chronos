...
int* numberOfLoop;
int* numberofContext;
//int currentContext; // TODO: removed
//int lengthOfContextMark; //TODO: removed
...
static void lookup_mlat(int orig, int dist, tcfg_node_t *bbi, int no_mlat) {
	int i, k, tmp, r;
	de_inst_t *inst;
	tcfg_edge_t *e;
...
static void bound_elog_len() {
	...
	max_elog_len[0] = 0;
	if (mlat_bbi[0] >= 0)
		max_elog_len[0] = pipe_iwin_size
				- (tcfg[0]->bb->num_inst - mlat_bbi[0]);
...
static void new_prolog(int log_set, tcfg_edge_t **path, int path_len,
		int num_skip) {
	int i, k, num = 0, num_mp = 0, tag, set, edge_id, max_num_mp, set_l2,
			tag_l2;
...
static void trav_backward(int log_set, tcfg_edge_t **path, int path_len,
		int code_len) {
	tcfg_node_t *bbi;
	tcfg_edge_t *e;
	int i;
...
static int new_epilog(int log_set, tcfg_edge_t **path, int path_len) {
	int i, k, num = 0, num1 = 0, m = 0, max_len, hit_mlat, set, tag, set_l2,
			tag_l2;
	tcfg_node_t *bbi;
	Queue queue;
	code_link_t *elog;
...
static int trav_forward(int log_set, tcfg_edge_t **path, int path_len,
		int code_len) {
	tcfg_node_t *bbi;
	tcfg_edge_t *e;
...
static void set_body_hitmiss(int edge_id, int hm_id, int contextMask) {
...
static void ctx_unit_time(int edge_id, int bp, int contextMask) {
...
#if 0
	printf("edge_id %d_%d context:%d\n", tcfg_edges[edge_id]->src->id,
			tcfg_edges[edge_id]->dst->id, contextMask);
#endif
	if (enable_icache) {
...
//		printf("edge %d_%d context:%d\n", tcfg_edges[edge_id]->src->id,
//				tcfg_edges[edge_id]->dst->id, contextMask);
		tcfg_node_t* src = tcfg_edges[edge_id]->src;
		tcfg_node_t* dst = tcfg_edges[edge_id]->dst;
		int num = dst->bb->num_inst;
		int i;
		for (i = 0; i < num; i++) {
			de_inst_t* inst = &(dst->bb->code[i]);
			printf("\tinst 0x%x: icf:%d", inst->addr, bodies[edge_id][i].ic_flag);
			if (enable_il2cache || enable_ul2cache) {
				printf(" icf2:%d", bodies[edge_id][i].ic_flag_l2);
			}
			printf("\n");
		}
#endif
	...
	if (enable_scp_dcache) {
		...
		for (i = 0; i < num; i++) {
			...
			if (iType == INST_LOAD || iType == INST_STORE) {
				printf("\tinst 0x%x: dcf:%d", inst->addr,
						bodies[edge_id][i].dc_flag);
				if (enable_il2cache || enable_ul2cache) {
					printf(" dcf2:%d", bodies[edge_id][i].dc_flag_l2);
				}
				if (iType == INST_STORE) printf(" [STORE]");
				else printf(" [LOAD]");
				printf("\n");
			}
		}
			...
			for (elog = epilogs[e->id]; elog != NULL; elog = elog->next) {
				...
				if (enable_scp_dcache) {
					set_prolog_data_cache_hitmiss(edge_id, plog->code, num_p,
							contextMask);
#if 0
					printf("edge %d_%d context:%d [PROLOG]\n",
							tcfg_edges[edge_id]->src->id,
							tcfg_edges[edge_id]->dst->id, contextMask);
					tcfg_node_t* src = tcfg_edges[edge_id]->src;
					tcfg_node_t* dst = tcfg_edges[edge_id]->dst;
					int i;
					for (i = num_p - 1; i >= 0; i--) {
						de_inst_t* inst = plog->code[i].inst;
						int iType = inst_type(plog->code[i].inst);
						if (iType == INST_LOAD || iType == INST_STORE) {
							printf("\tinst 0x%x: dcf:%d", inst->addr,
									plog->code[i].dc_flag);
							if (enable_il2cache || enable_ul2cache) {
								printf(" dcf2:%d",
										plog->code[i].dc_flag_l2);
							}
							if (iType == INST_STORE)
							printf(" [STORE]");
							else
...
#endif
					set_epilog_data_cache_hitmiss(edge_id, elog->code, num_e,
							contextMask);
				}
				/****************************************/
...
//										contextMask);
		...
		create_egraph(plog->code, num_p, NULL, 0, bodies[edge_id], num_b, bp,
				lp);
		t = est_egraph();
...
#if 0
	printf("WCET edge %d_%d context %d: %d ", tcfg_edges[edge_id]->src->id,
			tcfg_edges[edge_id]->dst->id, contextMask, *t_context);
	if (bbi->bb->num_d_inst > 0)
	printf("\tflag(%d)", bbi->bb->num_d_inst);
	printf("\n");
	int i;
	for (i = 0; i < bbi->bb->num_inst; i++) {
		mas_inst_t* inst = &(bodies[edge_id][i]);
		int iType = inst_type(bodies[edge_id][i].inst);
		if (iType == INST_LOAD || iType == INST_STORE) {
			printf("\tdc_flag:%d ", inst->dc_flag);
			printf(" l2:%d ", inst->dc_flag_l2);
			if (iType == INST_LOAD)
			printf("[LOAD]");
			else
			printf("[STORE]");
			...
			//	printf("ic_flag:%d ",inst->ic_flag);
		}
	}
#endif
}

static void est_start_unit() {
...
static void get_loop_affected_sets(int lp_id, tcfg_edge_t *mp_edge,
		int num_inst) {
	tcfg_edge_t *edge;
	tcfg_node_t *bbi;
	tcfg_elink_t *elink;
...
static void find_cond_exit(int lp_id, tcfg_edge_t *edge, int num_inst) {
	tcfg_elink_t *elink;
...
// misses
static void collect_affected_sets() {
	tcfg_node_t *bbi;
	tcfg_edge_t *edge;
	tcfg_elink_t *elink;
	loop_t *lp;
	...
	mp_affected_sets = (tcfg_elink_t ***) calloc(num_tcfg_loops,
			sizeof(tcfg_elink_t **));
	for (lp_id = 1; lp_id < num_tcfg_loops; lp_id++)
		mp_affected_sets[lp_id] = (tcfg_elink_t **) calloc(cache.ns,
				sizeof(tcfg_elink_t *));
...
static void mp_set_body_hitmiss(int edge_id, int hm_id, int set) {
...
static void ctx_mpmiss_time(int edge_id, int hm_id, int set, int bp) {
...
void pipe_analysis() {
	...
	for (i = 0; i < num_tcfg_edges; i++) {
		...
		tcfg_edge_t* e_in;
		printf("\t in %d: ", src->id);
		for (e_in = src->in; e_in != NULL; e_in = e_in->next_in) {
			tcfg_node_t* bbi = e_in->src;
			printf(" %d", bbi->id);
		}
		printf("\n");
		printf("\t in %d:", dst->id);
		for (e_in = dst->in; e_in != NULL; e_in = e_in->next_in) {
			tcfg_node_t* bbi = e_in->src;
			printf(" %d", bbi->id);
		}
		printf("\n");
		code_link_t* plog;
		for (plog = prologs[e->id]; plog != NULL; plog = plog->next) {
			int inst_num = plog->num_inst;
			int j;
			for (j = 0; j < inst_num; j++) {
				mas_inst_t* inst = &(plog->code[j]);
				printf("\t");
				printf("0x%x bbi_id:%d", inst->inst->addr, inst->bbi_id);
				printf("\n");
			}
			printf("\n");
		}
...
static void dump_xlogs(code_link_t **xlogs) {
	tcfg_node_t *src, *dst;
	int i, k;
	code_link_t *log;
	mas_inst_t *mas_inst;
...
void dump_units_times() {
	...
	for (edge_id = 0; edge_id < num_tcfg_edges; edge_id++) {
		e = tcfg_edges[edge_id];
		printf("%d.%d -> %d.%d\n", bbi_pid(e->src), bbi_bid(e->src),
				bbi_pid(e->dst), bbi_bid(e->dst));
		if (enable_icache) {
...
void dump_unit_time(int edge_id, int hm, int bp) {
	...
	printf("    %d.%d -> %d.%d (hm:%d bp:%d): %d\n", bbi_pid(e->src),
			bbi_bid(e->src), bbi_pid(e->dst), bbi_bid(e->dst), hm, bp, t);
}
...
void dump_edge_mp_times(int edge_id, int hm) {
	...
	printf("    %d.%d -> %d.%d (hm:%d): ", bbi_pid(e->src), bbi_bid(e->src),
			bbi_pid(e->dst), bbi_bid(e->dst), hm);
	for (set = 0; set < cache.ns; set++) {
		if (t[set] != 0)
...
void dump_context_stats() {
	...
	for (i = 0; i < num_tcfg_nodes; i++) {
		...
		printf("%3d: (%3d, %3d/%2d) %6d\n", i, num_plogs[i], num_elogs[i],
				max_elog_len[i], num_ctxs);
		total += num_ctxs;
	}
	printf("\n-----------------------\ntotal: %d\n", total);
...
#if 0
extern worklist_p* lpReqPS;
int isPersistent(saddr_p addr, loop_t* lp) {
	return 0;
	if (lp == NULL
	)
	return 0;
	while (lp != NULL) {
		worklist_p lpPS = lpReqPS[lp->id];
		for (; lpPS != NULL; lpPS = lpPS->next) {
			saddr_p m = (saddr_p) lpPS->data;
			if (m->blkAddr == addr->blkAddr && cmpSAddr(m, addr, lp->id) == 0) {
				return 1;
			}
		}
		lp = lp->parent;
...
void scpDumpTS(worklist_p tsSet) {
	if (tsSet == NULL) {
		printf(" NULL");
		return;
	} else {
