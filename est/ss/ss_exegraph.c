...
static range16_t fu_lat[] = { { 1, 1 }, // FUClass_NA = 0,
		{ 1, 1 }, // IntALU,
		{ 1, 4 }, // IntMULT,
		{ 1, 4 }, // IntDIV,
		{ 1, 2 }, // FloatADD,
...
static void scan_pred_normal(int curr) {
	int inst;
...
static void scan_pred_mpred(int curr) {
...
static void inst_coexists(int curr) {
	int inst, rob_num = 1, lsq_num = 0;

	if ((eg_insts_type[curr] == INST_LOAD)
			|| (eg_insts_type[curr] == INST_STORE))
		lsq_num = 1;
...
static void mpinst_coexists(int curr, int br_inst) {
	int i, inst, br_succ;
...
int getDstCtx(int ctx, int ctx_length, int bbi_id) {
	extern int*numberOfLoop;
	int l = numberOfLoop[bbi_id] - 1;
	while (l < ctx_length) {
		ctx = ctx >> 1;
		l++;
	}
...
static void add_inst(int inst) {
#if 0
	printf("bbi_id:%d  inst:0x%x ", eg_insts[inst]->bbi_id,
			eg_insts[inst]->inst->addr);
	if (enable_icache) {
		printf(" icf:%d", eg_insts[inst]->ic_flag);
		if (enable_il2cache || enable_ul2cache)
		printf(" icf2:%d", eg_insts[inst]->ic_flag_l2);
	}
	if (enable_scp_dcache) {
		if (eg_insts_type[inst] == INST_LOAD
				|| eg_insts_type[inst] == INST_STORE) {
			printf(" dcf:%d", eg_insts[inst]->dc_flag);
			if (enable_scp_dl2cache || enable_ul2cache)
			printf(" dcf2:%d", eg_insts[inst]->dc_flag_l2);
		}
	}
	printf("\n");
#endif
	int stage, fu, lo, hi, ic_flag, bbi_id, mblk_id, mblk_id_l2, ic_flag_l2,
			dc_flag, dc_flag_l2;
	egraph_node_t *node;
	loop_t *lp;
	/***********************************************************/
	...
	if ((ic_flag != IC_HIT)
			&& (inst < plog_len || inst >= (plog_len + body_len))) {
		bbi_id = eg_insts[inst]->bbi_id;
		mblk_id = eg_insts[inst]->mblk_id;
		if (enable_icache == 0
				|| get_mblk_hitmiss(tcfg[bbi_id], mblk_id, body_loop) == IC_HIT
				)
			ic_flag = IC_HIT;
	}
	...
	if ((ic_flag_l2 != IC_HIT)
		...
		if ((enable_il2cache == 0 || enable_ul2cache == 0)
				|| get_mblk_hitmiss_l2(tcfg[bbi_id], mblk_id_l2, body_loop)
						== IC_HIT
				)
			ic_flag_l2 = IC_HIT;
	}
...
#if 0
			printf("\tbbi:%d inst:0x%x IF=[%d,%d]\n", eg_insts[inst]->bbi_id,
					eg_insts[inst]->inst->addr, node->lat.lo, node->lat.hi);
...
//				extern int cache_dl1_lat;
//				if (eg_insts_type[inst] == INST_LOAD)
//					printf("[LOAD] (%d,%d)%d ", inst, stage,dc_flag);
//				else
//					printf("[STORE] (%d,%d)%d ", inst, stage, dc_flag);
//				printf("lat:[%d,%d] dl2_lat:%d %d ", node->lat.lo, node->lat.hi,
//						mem_lat[0], enable_scp_dl2cache);
//				if (inst < plog_len || inst >= (plog_len + body_len)) {
//					printf(" [nib]");
//				}
//				printf("\n");
//			}
...
/*
 static void add_inst(int inst) {
 int dc_flag = DC_HIT; //data cache: hit or miss
 ...
 if (eg_insts_type[inst] == INST_LOAD) {
 //dc_flag = dcLatClassify(eg_insts[inst], currentContext);
 dc_flag = DC_NC;
 if (dc_flag == DC_NC) {
 if (allHitFlag == FLAG_NC) {
 dc_flag = DC_NC;
 } else if (allHitFlag == FLAG_L2HIT) {
 dc_flag = DC_L2_HIT;
 } else if (allHitFlag == FLAG_HIT) {
 dc_flag = DC_HIT;
 }
 }
 } else {
 dc_flag = DC_NC_STORE; //TODO: store instruction
 }
 }
 } else {
 dc_flag = DC_HIT;
 ...
 if (enable_scp_dcache == 1) {
 if (dc_flag == DC_HIT) { //L1 data cache hit
 node->lat.hi = node->lat.lo = cache_dl1_lat; //cache_dl1_lat;
 } else if (dc_flag == DC_L2_HIT) { //L2 data cache hit
 node->lat.lo = cache_dl1_lat;
 node->lat.hi = cache_dl1_lat + cache_dl2_lat; //??? check here
 //printf("DC_L2_HIT:%d\n",node->lat.hi);
 } else if (dc_flag == DC_NC) { //cache miss -> access main memory
 node->lat.lo = cache_dl1_lat;
 if (enable_scp_dl2cache == 1)
 node->lat.hi = cache_dl1_lat + cache_dl2_lat
 + mem_lat[0]; //check here
 else
 node->lat.hi = cache_dl1_lat + cache_dl2_lat;
 //printf("DC_NC:%d\n",node->lat.hi);
 }else{
...
static void new_depend(egraph_node_t *src, egraph_node_t *dst, int low,
		int high, int normal) {
	egraph_edge_t *edge;
...
static void stage_order_depends() {
	int inst, stage;
...
static void succ_fetch_depends(int inst) {
...
static void succ_decode_depends(int inst) {
	int succ, edge_type, lo, hi;
	egraph_node_t *src, *dst;
...
static void succ_issue_depends(int inst) {
...
static void succ_commit_depends(int inst) {
	int succ, lo, hi;
	egraph_node_t *src, *dst;
...
static void fetch_width_depends_n(int inst, int succ) {
...
static void fetch_width_depends_m(int inst) {
...
static void fetch_width_depends(int inst, int succ) {
...
static void decode_width_depends_n(int inst, int succ) {
	egraph_node_t *src, *dst;
...
static void decode_width_depends_m(int inst) {
	int succ;
	egraph_node_t *src, *dst;
...
static void decode_width_depends(int inst, int succ) {
// first consider normal successor
...
static void issue_width_depends_n(int inst, int succ) {
	egraph_node_t *src, *dst;
...
static void issue_width_depends_m(int inst) {
	int succ;
	egraph_node_t *src, *dst;
...
static void issue_width_depends(int inst, int succ) {
// first consider normal successor
	issue_width_depends_n(inst, succ);
	if ((eg_insts[inst]->bp_flag != BP_CPRED)
			|| (succ == (inst + ruu_issue_width)))
		return;
...
static void commit_width_depends(int inst, int succ) {
	egraph_node_t *src, *dst;
...
static void ibuf_normal_depends(int inst, int succ) {
	egraph_node_t *src, *dst;
...
static void ibuf_mpred_depends(int inst, int succ) {
	egraph_node_t *src, *dst;
...
static void rob_depends(int inst, int succ) {
	egraph_node_t *src, *dst;
...
static void mark_data_dep(int src_id, int dst_id) {
	int inst, lbound;
...
// propagate depedence onto src's dependences
	...
	for (inst = eg_chain[src_id].pred; inst >= coexist[dst_id].lo; inst =
			eg_chain[inst].pred) {
...
static void create_bpred_depend(int src_id, int dst_id, int normal) {
	int lo, hi;
	egraph_node_t *src, *dst;
...
static void create_data_depend(int src_id, int dst_id, int normal) {
	egraph_node_t *src, *dst;

	if ((eg_insts_type[src_id] == INST_ICOMP)
			|| (eg_insts_type[src_id] == INST_FCOMP))
...
static int ready_earlier(int inst1, int inst2) {
	egraph_node_t *v1, *v2;
	egraph_edge_t *e1, *e2;
...
static int delay_type(egraph_edge_t *ce) {
	int earlier;
...
static void new_contend(egraph_node_t *src, egraph_node_t *dst, int normal) {
	egraph_edge_t *edge;
...
// arise if dst does not exist (e.g., the prediction is correct)
	if ((eg_insts[dst->inst]->bp_flag == BP_UNCLEAR)
			&& (eg_insts[src->inst]->bp_flag == BP_CPRED))
		normal = EG_COND_EDGE;
	edge->normal = normal;
...
static void inst_contends(int curr) {
	int inst, normal;
	egraph_node_t *src, *dst;
	...
	for (inst = eg_chain[curr].pred; inst >= 0; inst = eg_chain[inst].pred) {
		...
		else if (((src->fu == IntALU) || (src->fu == RdPort)
				|| (src->fu == WrPort))
				&& (dst->fu == IntALU || (dst->fu == RdPort)
						|| (dst->fu == WrPort))) {
...
void dump_coexist(int first, int last) {
	...
	for (i = first; i < last; i++)
		printf("coexist[%d]/%d: [%d, %d]\n", i, eg_insts[i]->bp_flag,
				coexist[i].lo, coexist[i].hi);
}
