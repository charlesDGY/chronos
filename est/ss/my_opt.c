...
int bimod_nelt = 1;
int bimod_config[1] = { /* bimod tbl size */4 };

/* 2-level predictor config (<l1size> <l2size> <hist_size> <xor>) */
int twolev_nelt = 4;
int twolev_config[4] = { /* l1size */1, /* l2size */128, /* hist */2, /* xor */
TRUE };

/* combining predictor config (<meta_table_size> */
int comb_nelt = 1;
int comb_config[1] = { /* meta_table_size */4 };
...
int btb_nelt = 2;
int btb_config[2] = { /* nsets */4, /* assoc */1 };
...
int mem_nelt = 2;
int mem_lat[2] = { /* lat to first chunk */30, /* lat between remaining chunks */
1 };
...
/* sudiptac ::: adding options for level 2 instruction cache
 * */
...
struct res_desc fu_config[] = { { "integer-ALU", 1, 0, { { IntALU, 1, 1 } } }, {
		"integer-MULT/DIV", 1, 0, { { IntMULT, 4, 4 }, { IntDIV, 4, 4 } } }, {
		"memory-port", 2, 0, { { RdPort, 1, 1 }, { WrPort, 1, 1 } } }, {
		"FP-adder", 1, 0, { { FloatADD, 2, 2 }, { FloatCMP, 2, 2 }, { FloatCVT,
				2, 2 } } }, { "FP-MULT/DIV", 1, 0, { { FloatMULT, 12, 12 }, {
		FloatDIV, 12, 12 }, { FloatSQRT, 12, 12 } } }, };
...
void sim_reg_options(struct opt_odb_t *odb) {
	...
	opt_reg_string(odb, "-cache:dl1",
			...
			"none", /* print */TRUE, NULL);

	opt_reg_int(odb, "-cache:dl1lat", "l1 data cache hit latency (in cycles)",
			&cache_dl1_lat, /* default */1, /* print */TRUE, /* format */NULL);
	...
	opt_reg_int(odb, "-tlb:lat", "inst/data TLB miss latency (in cycles)",
			&tlb_miss_lat, /* default */30, /* print */TRUE, /* format */NULL);
	...
	opt_reg_int(odb, "-res:imult",
			...
			fu_config[FU_IMULT_INDEX].quantity, /* print */TRUE, /* format */
			NULL);
	...
	opt_reg_int(odb, "-res:fpalu",
			...
			fu_config[FU_FPALU_INDEX].quantity, /* print */TRUE, /* format */
			NULL);
	...
	// register my options
	opt_reg_string(odb, "-run", "functionality {CFG EST}", &run_opt, /* default */
	"EST", /* print */TRUE, /* format */NULL);
...
int argc, char **argv) /* command line arguments */
{
	...
	else if (!mystricmp(pred_type, "nottaken"))
	{
		...
		if ((twolev_config[2] < 0) || (twolev_config[2] > 4))
			fatal(
					"Branch History Register (BHR) bits %d is not within [0, 4]\n",
					twolev_config[2]);
		...
		if ((1 << BHR) > BHT_SIZE)
			fatal(
					"The combination of Branch History Register (BHR) bits %d and \nBranch History Table size %d is invalid because 2^BHR > BHT \n",
					BHR, BHT_SIZE);
	}
#if 0
	...
	{
		if (sscanf(cache_dl1_opt, "%[^:]:%d:%d:%d:%c", name, &nsets_dl1,
				&bsize_dl1, &assoc_dl1, &c) != 5)
			fatal(
		...
		if (!mystricmp(cache_dl2_opt, "none"))
			enable_scp_dl2cache = 0;
#if 0
		cache_dl2 = NULL;
#endif
		else
		{
			if (sscanf(cache_dl2_opt, "%[^:]:%d:%d:%d:%c", name, &nsets_dl2,
					&bsize_dl2, &assoc_dl2, &c) != 5)
				fatal("bad l2 D-cache parms: "
						"<name>:<nsets>:<bsize>:<assoc>:<repl>");
			enable_scp_dl2cache = 1; //only instruction cache is enable
			/*
			 * TODO: consider unified cache
			 */
		}
	}
#if 0
	...
	else /* il1 is defined */
	{
		...
		if (!mystricmp(cache_il2_opt, "none"))
			...
			enable_il2cache = 1;
			enable_ul2cache = 1; // unified cache is enable
#if 0
					if (!cache_dl2)
			...
			enable_il2cache = 1;
			if (sscanf(cache_il2_opt, "%[^:]:%d:%d:%d:%c", name, &nsets_l2,
					&bsize_l2, &assoc_l2, &c) != 5)
