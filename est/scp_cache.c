...
loop_t* bbi_lp(tcfg_node_t *bbi) {
	...
	if (bbi->loop_id == -1)
		return loops[0]; //not in loop
	lp = loops[inf_loops[bbi->loop_id].loop_id];
	if (dbg) {
		fprintf(dbgMpa, " -> lp->id %d", lp->id);
		fflush(dbgMpa);
	}
	return lp;
}
...
int cmpSBlk(sblk_p sblk1, sblk_p sblk2) {
	if (sblk1->age != sblk2->age)
	return 1;
	if (sblk1->ys != sblk2->ys)
	return 1;
	return 0;
}
...
int addToYS(sblk_p acsBlk, saddr_p mAcc) {
	int i;
	int ysSize;
	if (acsBlk->age == EVICTED
	)
	return 0;
	ysSize = acsBlk->age - 1;
	for (i = 0; i < ysSize; i++) {
		if (acsBlk->ys[i]->blkAddr == mAcc->blkAddr)
		return 0;
	}
...
int singletonAccQuick(dat_inst_t* d_inst, int lpId) { //quick check, not formally proven, same result
	...
	if (exp->varNum == 0)
	return 1;//D is scalar access
	if (lpId == 0)
	return 0;//L_id is the outermost loop
	for (i = 0; i < exp->varNum - 1; i++) {
		if (exp->value[i].t != VALUE_CONST
		)
		return 0; //D is not loop-affine access
	}

	i = exp->varNum - 1; //induction variable of the inner most loop
	if (exp->value[i].t == VALUE_CONST && exp->value[i].val >= lpId) {
		if (exp->coef[i] < SIZE_OF_BLOCK
		)
		return 1;
	}
	return 0;
...
int singletonAcc(worklist_p *addrList, saddr_p tsM, dat_inst_t* d_inst,
		int lpId) {
	worklist_p addrNode;
	saddr_p mAcc;
	int count, cmp;
...
void PS_data_update(scp_acs acs_out, dat_inst_t *d_inst, loop_t *lp) {
	...
	int cmp, found;
	int singleton;
	worklist_p sblkNode, addrNode, olapNode, prvNode, agedNode, addr_set,
	addrList;
	saddr_p mAcc;
	...
	if (dbg) {
		fprintf(dbgMpa, " canRenew: %d", singleton);
	}
	if (dbg) {
#if 0
	...
	for (i = 0; i < MAX_CACHE_SET; i++) {
		...
		while (!isEmpty(newBlk[i])) {
			...
			if (acsBlk->m->blkAddr == mAcc->blkAddr) {
				cmp = cmpSAddr(acsBlk->m, mAcc, lp->id);
				if (cmp == EQUAL_TS) {
					if (singletonAcc(&addrList, acsBlk->m, d_inst, lp->id)
							== 1) { //formally proven approach
						clearYS(acsBlk);
					}
#if 0
				...
				if (acsBlk->m->blkAddr < mAcc->blkAddr) {
					ADD_NEW_BLK: tmpBlk = createSBlk(mAcc);
	...
	if (dbg) {
		for (i = 0; i < 1; i++) {
			fprintf(dbgMpa, "\nD[%d] ", i);
			printCacheSet(dbgMpa, acs_out[i]);
		}
	}
}
...
void analyze_loop_ps(loop_t *lp) {
	...
	/*if (lp->flags == LOOP_ANALYZED) {
	 fprintf(dbgMpa, " : lp analyzed");
	 return;
	 } else
	 lp->flags = LOOP_ANALYZED; //mark analyzed*/
	...
	lpTail = tcfg[num_tcfg_nodes - 1];
	if (dbg) {
		fprintf(dbgMpa, " [%d,%d]", lpHead->id, lpTail->id);
		fflush(dbgMpa);
	}
	...
	while (!p_queue_empty(&pQueue)) {
		...
		if (cmpLpOrder(hdLpId, blkLpId) == -1)
		continue;

		if (dbg) {
			fprintf(dbgMpa, "\n\nAnalyze bbi (%d,%d), L%d", bbi_pid(curNode),
					bbi_bid(curNode), blkLpId);
			fflush(dbgMpa);
		}
		// merge ACS of incoming edges
		changed = 0;
		for (in_edge = curNode->in; in_edge != NULL;
		...
		if (changed) {
			...
			for (out_edge = curNode->out; out_edge;
...
void getOuterPS(loop_t *lp) {
	...
	for (in_e = head->in; in_e; in_e = in_e->next_in) {
		...
		if (dbg) {
			fprintf(dbgMpa, "\nEnter L%d from (%d,%d)", lp->id, bbi_pid(src),
					bbi_bid(src));
			for (i = 0; i < MAX_CACHE_SET; i++) {
				fprintf(dbgMpa, "\nS[%d] ", i);
...
void estLpMissPS(loop_t *lp) {
		...
		for (; inNode; inNode = inNode->next) {
			...
			if (inBlk->m->blkAddr < reqBlk->blkAddr) {
				...
				if (cmpSAddr(inBlk->m, reqBlk, lp->id) == 0) {
					...
					found = 1;
					addToIncSet(reqBlk, &prvReqNode, &(lpReqPS[lp->parent->id]),
							lp);
					if (dbg) {
						fprintf(dbgMpa, " U:%x ", reqBlk->blkAddr);
					}
					break;
				} else {
					prvNode = inNode;
				...
				if (dbg) {
					fprintf(dbgMpa, " C:%x:%d ", reqBlk->blkAddr, lpMiss);
				}
				break;
			}
		}
	...
	if (1) {
		printf("\nIn L%d, Cold miss %d, Entry %d, Exec %d", lp->id,
				lp_coldms[lp->id], lpEntry, lp->exec);
		fflush(stdout);
	}
}
...
void estNodeAllMiss(tcfg_node_t* bbi) {
	...
	for (n_inst = 0; n_inst < bbi->bb->num_d_inst; n_inst++) {
		...
		if (isStoreInst(insn->op)) {
			...
			if (1) {
				printf("\n\nDataRef: ");
				printInstr(d_inst->insn);
				printf("maxPS: %d, maxMs: %d, maxExec: %d", num_PS,
						d_inst->max_miss, max_exec);
				fflush(stdout);
			}
			continue;
				...
				if (addrBlk->blkAddr == setBlk->m->blkAddr) {
					if (cmpSAddr(addrBlk, setBlk->m, lp->id) == 0) {
						if (setBlk->age == EVICTED) { //non-PS
							...
							if (dbg)
							addToWorkList(&miss_set, addrBlk);
						} else { //PS
		...
		if (max_miss < max_exec) {
			...
			for (setNode = LPS_req; setNode; setNode = setNode->next) {
				...
				if (dbg)
				addToWorkList(&PS_set, addrBlk);
			}
			while (!isEmpty(LPS_req))
			removeOneFromWorkList(&LPS_req);
		} else { //just all miss
			ALL_MISS: max_miss = max_exec;
			d_inst->max_miss = max_miss;
		}
		...
		if (dbg) {
			...
			fprintf(dbgMpa, "\n#PS: %d, max_miss: %d, max_exec: %d", num_PS,
					d_inst->max_miss, max_exec);
			fprintf(dbgMpa, "\nList of miss.blk :");
...
#endif
			while (!isEmpty(PS_set))
			removeOneFromWorkList(&PS_set);
			while (!isEmpty(miss_set))
			removeOneFromWorkList(&miss_set);
		}

		if (1) {
			printf("\n\nDataRef: ");
			printInstr(d_inst->insn);
			printf("\nmaxPS: %d, maxMs: %d, maxExec: %d", num_PS,
					d_inst->max_miss, max_exec);
			fflush(stdout);
		}
		PS_data_update(bbi->acs_out, d_inst, lp);
...
void updateTemporalScope() {
	...
	for (i = 0; i < num_tcfg_nodes; i++) {
		...
		for (j = 0; j < bbi->bb->num_d_inst; j++) {
			...
			dinst = dinst + j;
			worklist_p addrSet = dinst->addr_set;
			for (; addrSet != NULL; addrSet = addrSet->next) {
				saddr_p saddr = (saddr_p) addrSet->data;
				worklist_p tsSet = saddr->tsList;
				loop_t* lp = (loop_t*) bbi_lp(bbi);
				worklist_p headNode = NULL;
				worklist_p tailNode = NULL;
				for (; lp != NULL; lp = lp->parent) {
					ts_p ts = (ts_p) tsSet->data;
					if (lp->id == ts->loop_id)
					break;
					else {
						worklist_p newNode = malloc(sizeof(worklist_s));
				...
				if (headNode != NULL) {
					tailNode->next = saddr->tsList;
					saddr->tsList = headNode;
				}
			}
		}
	}
...
void scpCheckMem() {
	...
	for (i = 0; i < num_tcfg_nodes; i++) {
		tcfg_node_t* bbi = tcfg[i];
		loop_t* lp = bbi_lp(bbi);
		int depth = 0;
		while (lp != NULL) {
			depth++;
			lp = lp->parent;
		}
		printf("\nbbi:%d l%d l%d\n", bbi->id, bbi_lpId(bbi), bbi->loop_id);
		int j;
		for (j = 0; j < bbi->bb->num_d_inst; j++) {
			dat_inst_t* dinst = bbi->bb->d_instlist;
			dinst = dinst + j;
			insn_t* insn = (insn_t*) dinst->insn;
			printf("\t%x [load/store]\n", insn->addr);
			worklist_p addrNode = dinst->addr_set;
			for (; addrNode != NULL; addrNode = addrNode->next) {
				saddr_p saddr = (saddr_p) addrNode->data;
				int cs = GET_SET(saddr->blkAddr);
				worklist_p setNode = bbi->acs_out[cs];

				int depth2 = 0;
				worklist_p tsSet = saddr->tsList;
				printf("\t\t%x\t", saddr->blkAddr);
				while (tsSet != NULL) {
					depth2++;
					ts_p ts = (ts_p) tsSet->data;
				...
				if (depth != depth2) {
					printf("BUGG!!!\n");
					if (isPersistent(saddr, bbi_lp(bbi)))
					printf("PS");
					else
					printf("not PS");
					printf("\n");
...
void** cloneAddrSet() {
	void** addrarr = calloc(num_tcfg_nodes, sizeof(worklist_p*));
	int i;
	for (i = 0; i < num_tcfg_nodes; i++) {
		tcfg_node_t* bbi = tcfg[i];
		worklist_p* arr = calloc(bbi->bb->num_d_inst, sizeof(worklist_p));
		int j;
		for (j = 0; j < bbi->bb->num_d_inst; j++) {
...
void cpyAddr(void** addrarr) {
	int i;
	for (i = 0; i < num_tcfg_nodes; i++) {
		tcfg_node_t* bbi = tcfg[i];
		int j;
		worklist_p* arr = (worklist_p*) addrarr + i;
		for (j = 0; j < bbi->bb->num_d_inst; j++) {
			dat_inst_t* dat = bbi->bb->d_instlist;
			dat = dat + j;
			dat->addr_set = (worklist_p) arr + j;
		}
	}
}

scp_acs getacs(loop_t* lp, worklist_p acs_arr, worklist_p lpnode,
		tcfg_node_t* bbi) {
	while (lpnode != NULL) {
		loop_t* t = lpnode->data;
		if (t->id == lp->id)
		return acs_arr->data;
		acs_arr = acs_arr->next;
		lpnode = lpnode->next;
	}
	return NULL;
}
int scpisaddrPSinacs(saddr_p addr, scp_acs acs, loop_t* lp) {
	int cacheset = GET_SET(addr->blkAddr);
	worklist_p setnode = acs[cacheset];
	for (; setnode != NULL; setnode = setnode->next) {
		sblk_p sblk = (sblk_p) setnode->data;
		if (sblk->m->blkAddr == addr->blkAddr
				&& cmpSAddr(sblk->m, addr, lp->id) == EQUAL_TS) {
			if (sblk->age != EVICTED
			)
			return 1;
		}
...
//int scpispslp(saddr_p addr, loop_t* lp) {
...
//	}
//	int i;
//	for (i = head->id; i <= tail->id; i++) {
//		tcfg_node_t* bbi = tcfg[i];
//		scp_acs acs = getacs(lp, bbi->acs_in_arr, bbi->acs_in_lp, bbi);
//		if (scpisaddrPSinacs(addr, acs, lp) == 0)
//			return 0;
//		acs = getacs(lp, bbi->acs_out_arr, bbi->acs_out_lp, bbi);
//		if (scpisaddrPSinacs(addr, acs, lp) == 0)
//			return 0;
//	}
//	return 1;
//}
/*
 worklist_p scplpps(saddr_p addr, tcfg_node_t* bbi) {
 loop_t* lp = bbi_lp(bbi);
 worklist_p lpans = NULL;
 while (lp != NULL) {
 if (scpispslp(addr, lp) == 1)
 addToWorkList(&lpans, lp);
 lp = lp->parent;
 }
 return lpans;
 }

 void clone_acs(loop_t* lp) {
 tcfg_node_t* head = lp->head;
 tcfg_node_t* tail = lp->tail;
 if (tail == NULL) {
 tail = tcfg[num_tcfg_nodes - 1];
 ...
 for (i = head->id; i <= tail->id; i++) {
 tcfg_node_t* bbi = tcfg[i];
 scp_acs acs = calloc(MAX_CACHE_SET, sizeof(worklist_p));
 cpyACS(acs, bbi->acs_in);
 if (current_analysis == L1_DCACHE_ANALYSIS) {
 addToWorkList(&bbi->acs_in_arr, acs);
 addToWorkList(&bbi->acs_in_lp, lp);
 } else if (current_analysis == L2_DCACHE_ANALYSIS) {
 addToWorkList(&bbi->acsl2_in_arr, acs);
 addToWorkList(&bbi->acsl2_in_lp, lp);
 }

 acs = calloc(MAX_CACHE_SET, sizeof(worklist_p));
 cpyACS(acs, bbi->acs_out);
 if (current_analysis == L1_DCACHE_ANALYSIS) {
 addToWorkList(&bbi->acs_out_arr, acs);
 addToWorkList(&bbi->acs_out_lp, lp);
 } else if (current_analysis == L2_DCACHE_ANALYSIS) {
 addToWorkList(&bbi->acsl2_out_arr, acs);
 addToWorkList(&bbi->acsl2_out_lp, lp);
 }
 }
 }*/

/*
 int scpcmaddrlp(saddr_p addr, tcfg_node_t* bbi) {
 int miss = 0;
 worklist_p lpans = scplpps(addr, bbi);
 loop_t* pslp = NULL;
 if (lpans != NULL) {
 pslp = (loop_t*) lpans->data;
 }
 if (pslp != NULL) {
 if (pslp->parent == NULL
 )
 ...
 miss = estScopeSize(addr->tsList, pslp->parent->id);
 } else {
 miss = estScopeSize(addr->tsList, bbi_lpId(bbi));
 }
 return miss;
 }
 int scpcachemissref(worklist_p addrset, tcfg_node_t* bbi) {
 int miss = 0;
 for (; addrset != NULL; addrset = addrset->next) {
 miss += scpcmaddrlp(addrset->data, bbi);
 }
 return miss;
 }
 void scpcalcachmiss() {
 ...
 printf("DataRef: ");
 printInstr(insn);
 int max_miss = scpcachemissref(dat->addr_set, bbi);
 total_miss += max_miss;
 bbi->max_miss += max_miss;
 printf("\nMaxMiss: %d", max_miss);
 printf("\n\n");
 }
 }
...
int scp_cmp_saddr(saddr_p dst, saddr_p src, loop_t* lp) {
	...
	for (; dstnode != NULL; dstnode = dstnode->next) {
		...
		if (lp != NULL && scp_cmpLpOrder(ts->loop_id, lp->id) < 0)
		continue;
...
void uni_transform_ul2acs(tcfg_node_t* bbi, loop_t*lp) {
	...
	for (inst_id = 0; inst_id < bbi->bb->num_inst; inst_id++) {
		...
		/* Each instruction corresponds to two addresses. Update
		 * the instruction cache state accordingly */
		if (inst_chmc_l1[bbi->id][inst_id] == ALL_MISS) {
			/*CAC: A - always access to ul2 cache*/
			uni_update_inst(bbi->mpa_acs_out, GET_MEM(inst->addr));
			uni_update_inst(bbi->mpa_acs_out, GET_MEM(inst->addr+SIZE_OF_WORD));
		} else if (inst_chmc_l1[bbi->id][inst_id] == NOT_CLASSIFIED
				|| inst_chmc_l1[bbi->id][inst_id] == PS) {
			/*CAC: U - uncertainly access to ul2 cache*/
			/*first part*/
			scp_acs temp_acs = scp_createEmptyACS();
			scp_cpyACS(temp_acs, bbi->mpa_acs_out);
			uni_update_inst(bbi->mpa_acs_out, GET_MEM(inst->addr));
			uni_join(bbi->mpa_acs_out, temp_acs, lp);
			scp_discardACS(temp_acs);
			/*second part*/
			temp_acs = scp_createEmptyACS();
			scp_cpyACS(temp_acs, bbi->mpa_acs_out);
...
void scp_analyze_loop_ps(loop_t* lp, int analysis) {
...
#if 0
		if (scp_cmpLpOrder(innerLp->id, lp->id) > 0) //TODO: change here
		continue;
#endif
		if (isInner(innerLp->id, lp->id) == 0)
			continue;
		int j = 0;
	...
	while (scp_empty_queue(qHead, qTail) == 0) {
		...
		scp_acs acs_in = scp_createEmptyACS();
#if 0
		if (curnode->id == 5 && lp->id == 0) {
			printf("\n***************************************************\n");
			printf("curnode:%d lp:%d\n", curnode->id, lp->id);
			scp_print_ACS(curnode->mpa_acs_in, lp);
		}
#endif
		tcfg_edge_t* in_e;
		for (in_e = curnode->in; in_e != NULL; in_e = in_e->next_in)
			if (scp_cmpLpOrder(lp->id, loop_map[in_e->src->id]->id) >= 0) { //TODO: change here
				if (analysis == L1_DCACHE_ANALYSIS
					...
					if (curnode->id == 5 && lp->id == 0) {
						...
						printf("###################\n");
						printf("ACS of %d\n", in_e->src->id);
						scp_print_ACS(in_e->src->mpa_acs_out, lp);
						printf("###################\n");
						printf("\n");
						printf("before, curnode:%d lp:%d\n", curnode->id,
								lp->id);
						scp_print_ACS(acs_in, lp);
					}
#endif
...
#if 0
			printf("curnode:%d(out)\n", curnode->id);
			uni_print_ACS(curnode->mpa_acs_out);
#endif
		}
#if 0
		if (curnode->id == 5 && lp->id == 0) {
		...
		for (out_e = curnode->out; out_e != NULL; out_e = out_e->next_out)
			if (scp_cmpLpOrder(lp->id, loop_map[out_e->dst->id]->id) >= 0) { //TODO: change here
	...
	for (i = 0; i < num_tcfg_nodes; i++) {
		tcfg_node_t* bbi = tcfg[i];
		loop_t* innerLp = loop_map[bbi->id];
		if (isInner(innerLp->id, lp->id) == 0) //TODO: change here
			continue;
#if 0
		scp_acs acs;
		if (analysis == L1_DCACHE_ANALYSIS) {
			acs = scp_createEmptyACS();
			scp_cpyACS(acs, bbi->mpa_acs_in);
			addToWorkList(&(bbi->mpa_acsin_arr), acs);
			addToWorkList(&(bbi->mpa_lp_in), lp);

			acs = scp_createEmptyACS();
			scp_cpyACS(acs, bbi->mpa_acs_out);
			addToWorkList(&(bbi->mpa_acsout_arr), acs);
			addToWorkList(&(bbi->mpa_lp_out), lp);
		} else if (analysis == L2_DCACHE_ANALYSIS) {
			acs = scp_createEmptyACS();
			scp_cpyACS(acs, bbi->mpa_acs_in);
			addToWorkList(&(bbi->mpa_acsl2in_arr), acs);
			addToWorkList(&(bbi->mpa_lpl2_in), lp);

			acs = scp_createEmptyACS();
			scp_cpyACS(acs, bbi->mpa_acs_out);
			addToWorkList(&(bbi->mpa_acsl2out_arr), acs);
			addToWorkList(&(bbi->mpa_lpl2_out), lp);
		}
#endif

#if 0
		if (lp->id == 0) {
			if (analysis == L2_DCACHE_ANALYSIS) {
				printf("basic block %d lp:%d\n", bbi->id, lp->id);
				scp_print_ACS(bbi->mpa_acs_in, lp);
			} else if (analysis == UNIFIED_CACHE_ANALYSIS) {
				printf("basic block %d lp:%d\n", bbi->id, lp->id);
				uni_print_ACS(bbi->mpa_acs_in, lp);
				printf("\n");
			}
		}
#endif
...
#if 0
		if (analysis == L1_DCACHE_ANALYSIS && lp->id==0 && bbi->id==0) {
			printf("DUMP ACS of basic block %d loop level %d\n", bbi->id,
					lp->id);
			scp_print_ACS(bbi->mpa_acs_in, lp);
		}
		printf("\n");
...
#if 0
				char addr_st[50];
				sprintf(addr_st, "%x", inst->addr);
				if (bbi->id == 5 && strcmp(addr_st, "4002a8") == 0) {
					printf("DEBUGGING Basic block %d(lp:%d):\n",bbi->id,lp->id);
					scp_print_ACS(acs_it, lp);
					printf("\n");
				}
#endif
				worklist_p node;
		...
		/*****************************************************/
		/*
		 * (hit/miss) access classification
		 */
		int j;
		for (j = 0; j < bbi->bb->num_d_inst; j++) {
			...
			for (; node != NULL; node = node->next) {
				saddr_p addr = node->data;
				//printf("loop level %d bbi:%d mem:%x\n",lp->id,bbi->id,addr->blkAddr);
				if (scp_addrINacs(addr, bbi->mpa_acs_in) == 1) {
					addToWorkList(&(addr->psLoop), lp);
				}
			}
		}
		/*
		 * (PS/NC) classification for instruction cache
		 */
		if (analysis == UNIFIED_CACHE_ANALYSIS) {
			for (j = 0; j < bbi->bb->num_inst; j++) {
				categorize_ul2_inst_PS_NC(bbi->id, bbi->mpa_acs_in, j,
						&(bbi->bb->code[j]), lp);
			}
		}
#endif
...
void initACS() {
	...
	for (i = 0; i < num_tcfg_nodes; i++) {
		tcfg_node_t* bbi = tcfg[i];
		bbi->mpa_acsin_arr = NULL;
		bbi->mpa_acsout_arr = NULL;
		bbi->mpa_lp_in = NULL;
		bbi->mpa_lp_out = NULL;

		bbi->mpa_acsl2in_arr = NULL;
		bbi->mpa_acsl2out_arr = NULL;
		bbi->mpa_lpl2_in = NULL;
		bbi->mpa_lpl2_out = NULL;
...
void scp_test_cache_miss() {
	...
	for (i = 0; i < num_tcfg_nodes; i++) {
		...
		for (j = 0; j < bbi->bb->num_d_inst; j++) {
			...
			fprintf(fdbg, "inst:0x%s bb:%d cache_miss:%d lp:%d\n", insn->addr,
					bbi->id, cachemiss, loop_map[bbi->id]->id);
		}
	}
