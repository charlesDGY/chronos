...
static void tcfg_node_str(tcfg_node_t *bbi) {
	sprintf(str, "%d", bbi->id);
}

// b<i>
static void tcfg_node_term(FILE *fp, tcfg_node_t *bbi, char *prefix,
		char *suffix) {
	tcfg_node_str(bbi);
...
// b<i>.<hm>
static void tcfg_node_hm_term(FILE *fp, tcfg_node_t *bbi, int hm, char *prefix,
		char *suffix) {
	tcfg_node_str(bbi);
...
static void tcfg_edge_str(tcfg_edge_t *e, int bpred) {
...
static void tcfg_edge_term(FILE *fp, tcfg_edge_t *e, int bpred, char *prefix,
		char *suffix) {
...
static void est_unit_term(FILE *fp, tcfg_edge_t *e, int bpred, int hm,
...
static void bfg_node_str(bfg_node_t *bbb) {
...
static void bfg_node_term(FILE *fp, bfg_node_t *bbb, char *prefix, char *suffix) {
	bfg_node_str(bbb);
	sprintf(term, "%sc%s%s", prefix, str, suffix);
	new_term(fp);
}

static void bfg_edge_str(bfg_edge_t *e, int bpred) {
...
static void bfg_edge_term(FILE *fp, bfg_edge_t *e, int bpred, char *prefix,
		char *suffix) {
...
static void btg_edge_str(btg_edge_t *e) {
	char str1[32], str2[32];
...
// p<i>_<j>T/N
static void btg_edge_term(FILE *fp, btg_edge_t *e, char *prefix, char *suffix) {
	btg_edge_str(e);
	sprintf(term, "%sp%s%s", prefix, str, suffix);
	new_term(fp);
}

static void hm_str(int edge_id, int hm) {
	sprintf(str, "%d.%d", edge_id, hm);
}

// hm<i>.<j>
static void hm_term(FILE *fp, int edge_id, int hm, char *prefix, char *suffix) {
	hm_str(edge_id, hm);
	sprintf(term, "%shm%s%s", prefix, str, suffix);
	new_term(fp);
}

static void mpset_str(tcfg_edge_t *edge, int lp_id, int set) {
	char tmp_str[32];
...
// mp<lp>.<set>
static void mpset_term(FILE *fp, tcfg_edge_t *edge, int lp_id, int set,
		char *prefix, char *suffix) {
...
static void mpunit_str(tcfg_edge_t *edge, int hm, int set) {
	char tmp_str[32];
...
// d<j>_<i>.<hm>.<set>
static void mpunit_term(FILE *fp, tcfg_edge_t *edge, int hm, int set,
		char *prefix, char *suffix) {
...
static char map_bb_bbi(int proc, int bb, int first, int sign, int coeff) {
	int i, j;
	tcfg_nlink_t *nl;
	tcfg_node_t *bbi;
...
static char map_bb_bbi_context(int proc, int bb, int first, int sign, int coeff,
		int k) {
	tcfg_nlink_t *nl;
	tcfg_node_t *bbi;
	char sign_s[3] = "", tmp[3], coeff_s[8] = "", prefix_str[32];
	...
	for (count = 0, nl = bbi_map[proc][bb]; count < k && nl != NULL;
			count++, nl = nl->next)
		;
		...
		for (k = 0; k < num_contexts; k++) {
			...
			for (p = line; p != rhs; p++) {
				if (*p == 'c') { /* cfg node variable */
					...
					// vivy: check if mapping is successful
					nomatch = map_bb_bbi_context(proc, bb, first, sign, coeff,
							k);
					if (nomatch)
						break;
...
static void vstart_cost_term() {
	fprintf(filp, "%d d%s_0", start_time, start_str);
}

static void cost_term(int edge_id, int bpred) {
	int i, tm, num_hm;
	tcfg_edge_t *e;
	extern int l1, l2;
	...
	if (enable_dcache) {
		...
		fprintf(filp, " + %d d%s.0", L1_MISS_PENALTY * bbi->n_data_persistence,
				str);
		if (enable_ul2cache)
			fprintf(filp, " + %d d%s.0",
					L2_MISS_PENALTY * bbi->n_u1_data_persistence, str);
	}
#endif
	/* For abstract instruction cache analysis */
	if (enable_abs_inst_cache) {
		...
		if (enable_il2cache)
			fprintf(filp, " + %d d%s.0",
					L2_MISS_PENALTY * bbi->n_l2_persistence, str);
		else if (enable_ul2cache)
			fprintf(filp, " + %d d%s.0",
					L2_MISS_PENALTY * bbi->n_u1_persistence, str);
	}
}

// tcfg flow constraints for a bbi
// b<i> = sum{ d<i>_<j> } = sum{ d<j>_<i> }
static void tcfg_node_cons(tcfg_node_t *bbi) {
	tcfg_edge_t *e;
...
static void edge_mpset_cons(int lp_id, tcfg_edge_t *edge) {
	int set, affected = 0;
...
static void find_cond_exit(int lp_id, tcfg_edge_t *edge) {
	tcfg_elink_t *elink;
...
static void set_mpunit_cons(int bbi_id, int hm, int lp_id) {
	int set;
	tcfg_elink_t *elink;
	tcfg_edge_t *edge;
...
static void estunit_mpunit_cons(int bbi_id, int hm, int lp_id) {
	int set, first;
	tcfg_elink_t *elink;
	tcfg_edge_t *edge;
...
static void mp_cache_cons() {
	mpset_cons();
	mpunit_cons();
}

static void estunit_mpcost_func(int bbi_id, int hm, int lp_id) {
	int set, penalty;
	char prefix_str[16];
	tcfg_edge_t *edge;
	tcfg_elink_t *elink;
	...
	for (edge = tcfg[bbi_id]->in; edge != NULL; edge = edge->next_in) {
		for (set = 0; set < cache.ns; set++) {
			...
			if (elink == NULL
				...
				continue;
			penalty = mp_times[edge->id][hm][set] - cpred_times[edge->id][hm];
...
static void mpunit_cache_misses(int bbi_id, int hm, int lp_id) {
	int set, i, num_tags;
	tcfg_elink_t *elink;
	tcfg_edge_t *edge;
	char prefix_str[8];
 ...
 */
int infeas_nullifiers(assign_t **nullifier_list, int num_nullifiers, int pid,
		int ctxt) {
	int i, count;
	...
	for (i = 0; i < num_nullifiers; i++) {
		...
		if (ndlink == NULL
			...
			printf(
					"Unbalanced context: required %d; found %d for nullifier(%d::%d)\n",
...
int infeas_blockcount(tcfg_node_t *ib, int ctxt) {
	...
	if (ndlink == NULL
		...
		printf(
				"Unbalanced context: required %d for node(%d::%d); found %d for entry(%d::%d)\n",
				ctxt, ib->bb->proc->id, ib->bb->id, count - 1, lp->pid,
				lp->entry), exit(1);
	...
	for (i = 0; i < prog.num_procs; i++) {
		...
		for (j = 0; j < ip->num_bb; j++) {
			...
			for (k = 0; k < br->num_BA_conflicts; k++) {
				...
				int br_dst =
						(cf->conflict_dir == JUMP) ?
								br->bb->out_t->dst->id : br->bb->out_n->dst->id;
				...
				while (cflink != NULL && brlink != NULL) {
					...
					infeas_nullifiers(cf->nullifier_list, cf->num_nullifiers,
							pid, ctxt);
			...
			for (k = 0; k < br->num_BB_conflicts; k++) {
				...
				int cf_src = cfbb->id;
				int cf_dst =
						(cf->conflict_dir == JJ || cf->conflict_dir == JF) ?
								cfbb->out_t->dst->id : cfbb->out_n->dst->id;
				int br_dst =
						(cf->conflict_dir == JJ || cf->conflict_dir == FJ) ?
								br->bb->out_t->dst->id : br->bb->out_n->dst->id;
