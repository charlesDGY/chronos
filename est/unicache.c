...
static int checkForConflicts(tcfg_node_t* bbi, de_inst_t* cinst, int age,
...
static void transforml2InstCacheState(tcfg_node_t* bbi, int* change_flag,
		ANALYSIS_T type, int iteration_count) {
	de_inst_t* inst;
	int n_inst, bbi_n_inst;
	int base, imm, index;
	char* isa_name;
	ric_p addr;
	acs_p** acs_out = NULL;
...
static void transformInstCacheState(tcfg_node_t* bbi, int* change_flag,
	...
	for (n_inst = 0; n_inst < bbi->bb->num_inst; n_inst++) {
		...
		/* Save a copy to check the equality between the 
		 * previous state and the updated state. This is 
		 * important for the iterative computation */
		acs_out = (acs_p **)malloc(MAX_CACHE_SET * sizeof(acs_p *));
		memset(acs_out, 0, MAX_CACHE_SET * sizeof(acs_p *));
...
static void categorize_l2_inst_X_PS_NC(tcfg_node_t* bbi, de_inst_t* inst,
		int inst_id) {
	mem_blk_set_t temp;
	int h1, h2;
 ...
 * or non-classified (L2 cache) */
static void categorize_l2_inst_X_AH_NC(tcfg_node_t* bbi, de_inst_t* inst,
		int inst_id) {
	mem_blk_set_t temp;
	int h1, h2;
	int hc1, hc2;
 ...
 * or non-classified (L2 cache) */
static void categorize_l2_inst_X_AM_NC(tcfg_node_t* bbi, de_inst_t* inst,
		int inst_id) {
	mem_blk_set_t temp;
	int h1, h2;
	int hc1, hc2;
...
static void categorize_inst_AH_NC(tcfg_node_t* bbi, de_inst_t* inst,
		int inst_id) {
	mem_blk_set_t temp;
	int h1, h2;
	int hc1, hc2;
 ...
 * or non-classified (L1 cache) */
static void categorize_inst_PS_NC(tcfg_node_t* bbi, de_inst_t* inst,
		int inst_id) {
	mem_blk_set_t temp;
	int h1, h2;
...
/* For a given instruction address classify it as all miss
 * or non-classified (L1 cache) */
static void categorize_inst_AM_NC(tcfg_node_t* bbi, de_inst_t* inst,
		int inst_id) {
	mem_blk_set_t temp;
	int h1, h2;
	int hc1, hc2;
...
static void categorize_L2(int type) {
	tcfg_node_t* bbi;
	de_inst_t* inst;
	char* isa_name;
	int i, j;
 ...
 * or non-classified */
static void categorize_L1(int type) {
	tcfg_node_t* bbi;
	de_inst_t* inst;
	char* isa_name;
	int i, j;
...
static void analyze_abs_l2_instr_cache(int type) {
	int change_flag = 1;
	tcfg_edge_t* edge;
	int i, j, topoidx;
	int temp = 0;
...
static void analyze_abs_instr_cache(int type) {
	int change_flag = 1;
	tcfg_edge_t* edge;
	int i, j, topoidx;
	int temp = 0;
